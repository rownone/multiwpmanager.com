...
class ButtonColumn extends CButtonColumn
{
     ...
     */
    public $evaluateOption = false;
    public $evaluateLabel = false;
    /**
    ...
    public function renderDataCellContent($row, $data)
    {
        ...
        foreach($this->buttons as $id=>$button){
            if($this->evaluateOption){
                foreach($button['options'] as $opt=>$val){
                    if($opt!='class'){
                        $button['options'][$opt] = 
                                $this->evaluateExpression($val, array('row'=>$row,'data'=>$data));
                    }
                }
            ...
            if($this->evaluateLabel){
                $button['label'] = $this->evaluateExpression($button['label'], array('row'=>$row,'data'=>$data));
            }
